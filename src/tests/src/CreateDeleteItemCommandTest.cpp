...
[[nodiscatd]] constexpr bool
operator==(const LevelD::Thing& left, const LevelD::Thing& right)
{
    return left.id == right.id && left.x == right.x && left.y == right.y
           && left.tag == right.tag && left.flags == right.flags
           && left.metadata == right.metadata;
}

[[nodiscard]] LevelD::Thing CreateItem(unsigned id, unsigned x, unsigned y)
{
    return LevelD::Thing { .id = id,
                           .tag = id,
                           .x = x,
                           .y = y,
                           .flags = 0,
...
TEST_CASE("[Create/DeleteItemCommand]")
{
    ...
    {
        auto command = CreateItemCommand(items, CreateItem(69, 24, 42));
        command.exec();
    ...
    {
        auto command = DeleteItemCommand(
            items,
            {
                9,
                3,
