...
Utilities::createChessboardTexture(const sf::Color& accentColor)
{
    sf::Image img;
    img.create(2, 1);
...
{
    return sf::Vector2i(std::max(0, v.x), std::max(0, v.y));
}

std::optional<TileRect> Utilities::unifyRects(
    const std::optional<TileRect>& a, const std::optional<TileRect>& b) noexcept
{
    ...
    const float arrowThickness)
{
    ...
    const auto size = dgm::Math::getSize(forward);
    if (size == 0.f) return sf::ConvexShape();

    const auto normal = sf::Vector2f(-forward.y, forward.x) / size;
    const auto normal45 = dgm::Math::getRotated(normal, 45);
    const auto normal90 = dgm::Math::getRotated(normal, 90);
    const auto normal135 = dgm::Math::getRotated(normal, 135);
    ...
    auto mirrorPoint = [&normal,
                        line_C](const sf::Vector2f& vec) -> sf::Vector2f
    {
        // Don't need to divide this by anything - normal is unit vector
        const float distance = (normal.x * vec.x + normal.y * vec.y + line_C);
    ...
    const auto innerArrorCorner = toF + normal90 * arrowThickness * SQRT_2
                                  + normal45 * arrowThickness * SQRT_HALF;
    const auto bottomCorner =
        sf::Vector2f(from) + normal * arrowThickness / 2.f;
