...
sf::Vector2f
Camera::getWorldCoordinates(const sf::Vector2f& viewCoordinates) const
{
    auto canvasPos = canvas->getPosition();
    auto difference = currentView.getCenter()
                      - canvas->getDefaultView().getCenter() * currentZoom
                      - sf::Vector2f(canvas->getPosition()) * currentZoom;
