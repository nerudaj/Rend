...
tgui::Panel::Ptr _getRowBackground(unsigned y, const std::string& tooltip)
{
    ...
    row->setToolTip(tt);
    row->getRenderer()->setBackgroundColor(
        y % 2 == 0 ? ROW_BGR_DARK : ROW_BGR_LIGHT);
    return row;
}
...
tgui::Label::Ptr _getLabel(const std::string& label)
{
    ...
    l->setSize(LABEL_WIDTH.c_str(), "100%");
    l->setPosition(LABEL_LEFT_MARGIN.c_str(), "0%");
    ...
    bool enabled)
{
    ...
    checkbox->setPosition(VALUE_LEFT_MARGIN.c_str(), "0%");
    checkbox->setChecked(data);
    checkbox->setEnabled(enabled);
    checkbox->onChange([&data](bool newVal) { data = newVal; });
...
void DialogBuilderHelper::addOption(
    ...
    bool tag)
{
    _addOptionUint(target, token, label, tooltip, data, ypos, enabled, tag);
}
    ...
    unsigned ypos)
{
    ...
    combo->setSize(VALUE_WIDTH.c_str(), "100%");
    combo->setPosition(VALUE_LEFT_MARGIN.c_str(), "0%");

    for (auto&& value : values)
        combo->addItem(value);
    combo->onItemSelect([combo, onChangeCallback]
